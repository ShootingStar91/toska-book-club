...
export async function bookSuggestionsRoute(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: CreateBookSuggestionRequest }>('/', {
    ...
    try {
      ...
      const result = await createBookSuggestion(request.user.userId, request.body);
      return reply.status(201).send(result);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to create book suggestion';
      ...
      if (errorMessage.includes('only allowed during') || errorMessage.includes('deadline has passed')) {
  ...
  fastify.get<{ Params: { cycleId: string } }>('/cycle/:cycleId', {
    preHandler: [authenticateToken]
  }, async (request, reply) => {
    try {
      ...
      return reply.status(500).send({ error: 'Failed to fetch book suggestions' });
    }
  });

  // GET /book-suggestions/my/:cycleId - Get current user's suggestion for a cycle
  fastify.get<{ Params: { cycleId: string } }>('/my/:cycleId', {
    preHandler: [authenticateToken]
  }, async (request, reply) => {
    try {
      if (!request.user) {
      ...
      const { cycleId } = request.params;
      const suggestion = await getUserSuggestionForCycle(request.user.userId, cycleId);
      
      if (!suggestion) {
        return reply.status(404).send({ error: 'No suggestion found for this cycle' });
      }
      
      return reply.status(200).send(suggestion);
    } catch {
      return reply.status(500).send({ error: 'Failed to fetch user suggestion' });
    }
  });
}