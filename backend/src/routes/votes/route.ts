...
export async function votesRoute(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: SubmitVotesRequest }>('/', {
    ...
    try {
      ...
      if (errorMessage.includes('invalid') || errorMessage.includes('not part of')) {
  ...
  fastify.get<{ Params: { cycleId: string } }>('/results/:cycleId', {
    preHandler: [authenticateToken]
  }, async (request, reply) => {
    try {
      const { cycleId } = request.params;
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to fetch vote results';
